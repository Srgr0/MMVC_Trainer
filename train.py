    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info('Train Epoch: {} [{:.0f}%] [{}] lr: {:.6f}'.format(
                    epoch,
                    100. * batch_idx / len(train_loader),
                    global_step, lr))
                logger.info(f"Losses: disc={loss_disc:.3f}, gen={loss_gen:.3f}, fm={loss_fm:.3f}, mel={loss_mel:.3f}, dur={loss_dur:.3f}, kl={loss_kl:.3f}")
