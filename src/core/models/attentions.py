...
class FFN(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_1 = nn.Conv1d(in_channels, filter_channels, kernel_size, padding=self.padding)
        self.conv_2 = nn.Conv1d(filter_channels, out_channels, kernel_size, padding=self.padding)
        self.drop = nn.Dropout(p_dropout)
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        attn_mask = x_mask.unsqueeze(2) * x_mask.unsqueeze(-1) if x_mask is not None else None
